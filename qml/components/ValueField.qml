...
Item {
    ...
    function formatted(v){if(unit==="Hz"&&v>=1000)return(v/1000).toFixed(v>=10000?1:2)+"k";return Number(v).toFixed(decimals)}
    ...
    Rectangle {
        ...
        MouseArea {
            ...
            onPressed:function(event){root.forceActiveFocus();root.dragging=true;root.pressY=event.y;root.pressValue=root.value}
            onPositionChanged:function(event){if(!pressed)return;var fine=(event.modifiers&Qt.ShiftModifier)!==0,pixelsPerStep=fine?16:5;root.valueEdited(root.quantize(root.pressValue+(root.pressY-event.y)/pixelsPerStep*root.step,fine))}
